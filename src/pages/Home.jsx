...
const testemunhos = [
  {
    ...
    imgBugged: imgReview1, // <- ALTERAR PARA imgReview1Bugged quando a tiveres
  ...
  },
  {
    ...
    imgBugged: imgReview2, // <- ALTERAR PARA imgReview2Bugged quando a tiveres
  ...
  },
  {
    ...
    imgBugged: imgReview3, // <- ALTERAR PARA imgReview3Bugged quando a tiveres
...
function Home({ isBugged }) {
  ...
  const handleIniciarUrSync = () => {
    if (isBugged) {
      Swal.fire({
        ...
        text: "Eles monitorizam cada passo teu. NÃO LHES DÊS OS TEUS DADOS.",
        icon: "error",
        background: "#0a0a0a",
        color: "#FF0000",
  ...
  return (
    <div className="home-container">
      ...
      {/* BLOCO 2: Headline e CTA (Mais geral e corporativo) */}
      <section className="hero-text-block">
        <h1>{isBugged ? 'Eles controlam tudo.' : 'Sabemos o que precisa antes de saber que precisa.'}</h1>
        <p>
          {isBugged 
        ...
        <button onClick={handleIniciarUrSync} className="btn-primary" style={{ backgroundColor: isBugged ? 'red' : 'black' }}>
          {isBugged ? 'CANCELAR SINCRONIZAÇÃO' : 'Otimizar Agora'}
        </button>
      </section>
          ...
          {listaProdutos.map((produto) => (
            ...
            >
              <div className="product-info">
                ...
                <p>{isBugged ? 'ACESSO RESTRITO' : produto.slogan}</p>
              </div>
            </Link>
          ))}
        </div>

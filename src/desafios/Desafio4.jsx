  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 md:p-6 font-mono text-center transition-colors duration-500 ${isBugged ? 'bg-neutral-950 text-red-600' : 'bg-white text-neutral-900'}`}>
      ...
      <div className="max-w-4xl w-full border border-current p-6 md:p-8 rounded-lg shadow-2xl bg-black relative overflow-hidden">
        ...
        <h1 className="text-2xl md:text-3xl font-bold mb-4 border-b border-current pb-4 text-emerald-600">
        ...
        {!desbloqueado ? (
          <div className="space-y-6 md:space-y-10">
            ...
            <p className={`text-lg md:text-xl font-bold min-h-[3rem] md:h-8 flex items-center justify-center ${isBugged ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}>
              {mensagem}
            </p>

            <div className="min-h-[6rem] md:h-24 flex items-center justify-center">
              {isBugged && (
            ...
            <div className="bg-neutral-900 p-5 md:p-8 rounded border border-neutral-800 text-lg md:text-3xl text-left leading-relaxed md:leading-loose select-none shadow-inner">
              <span className="text-white">A UrWell é a nossa </span>
              <button 
                onClick={() => alterarPalavra(0)}
                className={`inline-block px-3 py-1 md:px-4 md:py-1 mx-1 md:mx-2 my-1 font-bold border-b-4 transition-colors ${indices[0] === 0 ? 'text-emerald-500 border-emerald-500 bg-emerald-900/20' : indices[0] === 2 ? 'text-red-500 border-red-500 bg-red-900/20' : 'text-yellow-500 border-yellow-500 bg-yellow-900/20'}`}
              >
                [ {palavra1[indices[0]]} ]
              </button>
              
              <span className="text-white">. A ignorância é </span>
              <button 
                onClick={() => alterarPalavra(1)}
                className={`inline-block px-3 py-1 md:px-4 md:py-1 mx-1 md:mx-2 my-1 font-bold border-b-4 transition-colors ${indices[1] === 0 ? 'text-emerald-500 border-emerald-500 bg-emerald-900/20' : indices[1] === 2 ? 'text-red-500 border-red-500 bg-red-900/20' : 'text-yellow-500 border-yellow-500 bg-yellow-900/20'}`}
              >
                [ {palavra2[indices[1]]} ]
              </button>
              ...
              <button 
                ...
                className={`inline-block px-3 py-1 md:px-4 md:py-1 mx-1 md:mx-2 my-1 font-bold border-b-4 transition-colors ${indices[2] === 0 ? 'text-emerald-500 border-emerald-500 bg-emerald-900/20' : indices[2] === 2 ? 'text-red-500 border-red-500 bg-red-900/20' : 'text-yellow-500 border-yellow-500 bg-yellow-900/20'}`}
              >
                [ {palavra3[indices[2]]} ]
              </button>
            ...
            <button 
              ...
              className={`w-full py-4 md:py-6 text-xl md:text-2xl font-bold rounded-lg border-4 transition-transform active:scale-95 uppercase ${
                isBugged 
          ...
          <div className="space-y-6 text-green-500 pt-8 animate-in fade-in zoom-in duration-700">
            <h2 className="text-3xl md:text-4xl font-bold animate-pulse">CENSURA DESTRUÍDA.</h2>
